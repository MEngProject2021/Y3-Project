...
def upload_file(file):
    object_name = file
    response = s3_client.upload_file(file, settings.AWS_STORAGE_BUCKET_NAME, object_name)

    
def upload_s3(images_path):
    # print("upload to s3 2123123123123123213", file=sys.stderr)
    # print(images_path, file=sys.stderr)
...
class TaskViewSet(viewsets.ViewSet):
    ...
    def commit(self, request, pk=None, project_pk=None):
        ...
        # upload to S3
        upload_s3(task.find_all_files_matching(r'.*\.(jpe?g|tiff?|hdr?)$'))
        
        print(task.file_type, file=sys.stderr)

        if task.images_count < 2 and task.file_type == "image":
            raise exceptions.ValidationError(detail=_("You need to upload at least 2 images before commit"))

        
        if task.file_type == "hdr":
            print("do not process task", file=sys.stderr)
            task.status = status_codes.COMPLETED

        else:
            print("process task", file=sys.stderr)
            worker_tasks.process_task.delay(task.id)
        task.save() 
        serializer = TaskSerializer(task)
    ...
    @action(detail=True, methods=['post'])
    def download(self, request, pk=None, project_pk=None):
        print("download lalallallalla", file=sys.stderr)
        get_and_check_project(request, project_pk, ('change_project', ))
        try:
            task = self.queryset.get(pk=pk, project=project_pk)
        except (ObjectDoesNotExist, ValidationError):
    ...
    @action(detail=True, methods=['get'])
    def hyperspectral_retrive(self, request,  pk=None, project_pk=None):
        """
        Retrive hyperspectral image
        """
        try:
            task = self.queryset.get(pk=pk, project=project_pk)
        except (ObjectDoesNotExist, ValidationError):
        ...
        for file in os.listdir(task_path):
            if ".hdr" in file:
                file_name = file
                hdr_file_path = os.path.join(task_path, file)
                print(hdr_file_path, file=sys.stderr)
                
        url = "http://localhost:8090/api/generate_image"
        r = requests.post(url)
        
        print(r, file=sys.stderr)
        # img = envi.open(hdr_file_path, image=hdr_file_path)
        ...
        # print(rows, samples, bands, file=sys.stderr)
        # img_file_path = os.path.join(task_path, 'rgb.jpg')
        # save_rgb(img_file_path, img)
        
        return Response({"img_file_path": hdr_file_path,  }, status=status.HTTP_200_OK)
