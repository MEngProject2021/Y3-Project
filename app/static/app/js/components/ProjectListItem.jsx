...
class ProjectListItem extends React.Component {
  ...
  componentDidMount(){
    ...
    if (this.hasPermission("add")){
      ...
      this.dz.on("addedfiles", files => {
          let totalBytes = 0;
          let file_type = [... files[0]["upload"]["filename"].split(".")].pop();
        ...
        .on("queuecomplete", () => {
            ...
            if (remainingFilesCount === 0){
                ...
                this.setUploadState({
                    ...
                    uploading: false,
                    error: interpolate(_('%(count)s files cannot be uploaded. As a reminder, only images (.jpg, .tif, .png) , GCP files (.txt) and Hyperspectral files (.hdr) can be uploaded. Try again.'), { count: remainingFilesCount })
                });
            }
        })
        .on("reset", () => {
  ...
  handleTaskSaved(taskInfo){
    ...
    // console.log(this.state.upload.file_type)
    ...
    const formData = {
        ...
        partial: true,
        file_type: this.state.upload.file_type === "hdr" ? "hdr" : "image"
    };

    if (taskInfo.resizeMode === ResizeModes.YES && this.state.upload.file_type !== "hdr"){
        ...
        <div className="row">
          ...
          {this.state.upload.editing ? 
            <NewTaskPanel
              ...
              showResize={this.state.upload.file_type !== "hdr"}
              getFiles={() => this.state.upload.files }
              file_type={this.state.upload.file_type}
            />
          : ""}
