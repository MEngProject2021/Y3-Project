...
class TaskListItem extends React.Component {
  ...
  render() {
    ...
    // const name = task.name !== null ? task.name : interpolate(_("Task "), { number: task.id });
    ...
    if (this.state.expanded){
      ...
      if (task.status === statusCodes.COMPLETED){
        ...
        if (task.file_type === "image"){
          // addActionButton(" " + _("S3 Download"), "btn-primary", "fa fa-download", () => {
          //   // location.href = `/map/project/${task.project}/task/${task.id}/`;
          //   $.ajax({
          //     url: `/api/projects/${this.state.task.project}/tasks/${this.state.task.id}/download/`,
          //     contentType: 'application/json',
          //     // data: JSON.stringify(formData),
          //     dataType: 'json',
          //     type: 'POST'
          ...
          addActionButton(" " + _("View Hyperspectral"), "btn-primary", "fa fa-cube", () => {
            location.href = `/hyperspectral/project/${task.project}/task/${task.id}/`;
            console.log(" Hyperspectral view")
          });
        }
      ...
      actionButtons = (<div className="action-buttons">
            {/* {task.status === statusCodes.COMPLETED && task.file_type === "image"?
            ...
            <div className="col-md-12 no-padding">
              <table className="table table-condensed info-table">
                <tbody>
                  ...
                  {/* {Array.isArray(task.options) && task.file_type === "image" ? 
                  <tr>
