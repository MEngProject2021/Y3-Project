...
@login_required
def hyperspectral_display(request, project_pk=None, task_pk=None):
    title = _("Hyperspectral Image View")
    if project_pk is not None:
        project = get_object_or_404(Project, pk=project_pk)
        if not request.user.has_perm('app.view_project', project):
            raise Http404()
        ...
        if task_pk is not None:
            ...
            print(task.get_hyperspectral_display_params(), file=sys.stderr)
            title = task.name 
        else:
            raise Http404()
    return render(request, 'app/hyperspectral_dispay.html', {"title":  title, 
                            "params": {
                                "task": json.dumps(task.get_hyperspectral_display_params()),
                            }.items()
                    })
...
# @login_required
def dronedeploy(request):
    return render(request, 'app/dronedeploy.html', {'title': _('Drone Deploy Dashboard'), 'version': settings.VERSION})
